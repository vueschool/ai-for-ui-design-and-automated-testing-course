...
1. **Generate the OG Image**
   ...
   - **Template**: `image-templates/blog-post-og-template-no-text.png`
   - **Subject**: `public/blog/[post-slug]-subject-1x1.png`
   - **Output**: `public/blog/[post-slug]-og-text-only.png`
   - **Ratio**: 16:9
   ...
   **Prompt**:
   ...
   2. Keep the logo in the bottom right corner of the right column as it is in the template.
   ...
   **Example Command**:
   ...
   @generate-image.sh \
     ...
     --input-image image-templates/blog-post-og-template-no-text.png \
...
2. You MUST read and evaluate the generated image for quality:
  ...
  2. Is the logo in the bottom right corner of the right column the same as the template?
3. If it is not good enough, generate again with adjustments to the prompt as required.

4. Add Text to the OG Image

   Extract the `title` and `subtitle` from the blog post markdown file

   Run the `add-text-to-og.sh` script with the required arguments.

   **Example Command**:
   ```bash
   ./add-text-to-og.sh \
     --input public/blog/[post-slug]-og-text-only.png \
     --output public/blog/[post-slug]-og.png \
     --title "My Blog Post Title" \
     --subtitle "My Blog Post Subtitle"
   ```
